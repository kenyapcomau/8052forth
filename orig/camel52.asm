...
CYGNAL  equ     0       ; 1=Cygnal C8051F000; 0=generic 8052
...
; Forth linkage
link	set	0
IMMED	equ	0FEh   ; flag for Immediate word
NONIMMED	equ	0FFh

; 8051 EQUATES
$MOD8F000  		; Include regsiter definition file.
dr1	equ	01h
dr2	equ	02h   ; r2-r5 accessed as
dr3	equ	03h   ; direct registers;
dr4	equ	04h   ; required for PUSH and
dr5	equ	05h   ; POP instructions.
dr6	equ	06h   ; Assumes register bank 0
dr7	equ	07h   ; is selected.
...
userarea equ	08h      ; base of user area
dataram	equ	14h      ; where data goes (internal RAM)

lstack	equ	80h
padbufr	equ	8Ah      ; NO PAD AREA IS DEFINED FOR THE USER
termbufr equ	8Ah      ; base of terminal input buffer
rstack	equ	0B2h     ; initial return stack pointer
pstack	equ	0ffh     ; initial data stack pointer    *** FIX .S,DEPTH ***
...
; Initial RAM & ROM pointers for CamelForth.
        IF CYGNAL
coderam	equ	02000h    ; where new code goes  (Cygnal)
        ELSE
coderam	equ	0E000h    ; where new code goes  (generic 8052)
        ENDIF
...
GREEN_LED equ	P1.6 	; Port I/O pin connected to Green LED.
	...
	org	0B3h	; End of interrupt vector space. (Cygnal)
reset:  ; Cygnal C8051F initialization
	...
	mov 	PRT0CF,#01010110b  	; TX, MOSI, SCK & SC all outputs
	mov 	PRT1CF,#01110001b  	; CEX0 & green LED outputs

	; Initialize LED to OFF
	clr	GREEN_LED
	; UART setup
	mov	RCAP2L,#0FAh		; Autoload value for 57.6Kbaud
	mov	RCAP2H,#0FFh		; with 11.0592 MHz XTAL
	orl 	T2CON,#00110100b	; T2 is Baudrate gen. RCLK & TCLK =1
	orl	SCON,#01110000b   	; Serial Mode 1
	; Wait for XTAL stable and valid

osc_wait:
	mov	a,OSCXCN
	jnb acc.7, osc_wait
	orl	OSCICN,#00001000b	;select extn XTAL osc as sys clock
	anl	OSCICN,#NOT(00000100b) 	; disable internal oscillator

        ; Flash Memory Timing
	mov FLSCL,#88h         ; 6.4 MHz <= System Clock < 12.8 MHz
...
reset:  ; Generic 8051 initialization
	mov ie,#0       ; disable all irpts
        ...
        mov tmod,#20h   ; T1 mode 2, T0 mode 0
        mov th1,#0fdh   ; 9600 baud @ 11.0592 MHz
        setb tcon.6     ; enable timer 1
        mov scon,#52h   ; UART mode 1 (8-bit)
...
;C DUP      x -- x x       duplicate top of stack
        drw link
link	set	$+1   ; push low byte of TOS
	db	NONIMMED,3,'DUP'
DUP:
...
MAIN:   lcall PLUSWRITE
	lcall XISQUOTE
       DB 35,'8052 CamelForth v1.4  19 Mar 2001'
       DB 0dh,0ah
        lcall ITYPE
...
; DO NOT delete!
lastword equ	link      ; NFA of final word
...
raminit:
	...
	; RAM 14-7F = Application variables (available to user)
        ...
        IF CYGNAL
raminitsize equ 78h     ; for Cygnal, always load RAM 08-7F
        ELSE
raminitsize equ $-raminit
        ENDIF
